...
tGnssReceiver::tStateOperation::tStateOperation(tGnssReceiver* obj, const std::chrono::time_point<tClock>& startTime, bool noData)
	:tState(obj), m_StartTime(startTime), m_NoData(noData)
{
	m_pObj->m_pLog->WriteLine(true, utils::tLogColour::Default, "tStateOperation");
}

tGnssReceiver::tStateOperation::tStateOperation(tGnssReceiver* obj, const std::chrono::time_point<tClock>& startTime)
